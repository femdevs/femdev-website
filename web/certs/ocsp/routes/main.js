...
class Server {
	constructor(options) {
		...
		const hash = (alg, v) => crypto.createHash(alg).update(v).digest('hex');
		this.certID = Object.fromEntries(
			Object.keys(OCSPUtils.digest)
				.map(data => [data, {
					issuerNameHash: hash(data, Name.encode(this.cert.subject, 'der')),
					issuerKeyHash: hash(data, this.cert.subjectPublicKeyInfo.subjectPublicKey.data),
				}]),
		);
		this.certs = {};
	}
				...
				cb(
					...
					OCSPResponse.encode({
						...
						responseBytes: {
							...
							response: BasicOCSPResponse.encode({
								tbsResponseData,
								signatureAlgorithm: {
									algorithm: OCSPUtils.signRev.sha512WithRSAEncryption,
								},
								signature: {
									unused: 0,
									data: crypto
										.createSign('sha512WithRSAEncryption')
										.update(ResponseData.encode(tbsResponseData, 'der'))
										.sign(this.key),
								},
							}, 'der'),
						},
