...
.album-cover::before {
    ...
    z-index: 0;
        ...
        if (spotify.playing) {
            ...
            document.getElementById('current-time').textContent = `${Math.floor(currentTime / 60)}:${pad(currentTime % 60)}`;
            document.getElementById('duration-time').textContent = `${Math.floor(duration / 60)}:${pad(duration % 60)}`;
        } else {
